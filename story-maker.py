...
def extract_snippets(input_video_path, output_folder, aspect_ratio, snippet_count=3):
    ...
    for i in range(1, snippet_count + 1):
        ...
        if target_width <= video_width:  # Crop width
            crop_x1 = (video_width - target_width) / 2
            crop_x2 = crop_x1 + target_width
            crop_y1, crop_y2 = 0, video_height
        else:  # Crop height
            crop_y1 = (video_height - target_height) / 2
            crop_y2 = crop_y1 + target_height
            crop_x1, crop_x2 = 0, video_width

        # Apply cropping
        snippet = crop(snippet, x1=crop_x1, y1=crop_y1, x2=crop_x2, y2=crop_y2)

        # Construct output file name
        base_name = os.path.splitext(os.path.basename(input_video_path))[0]
        aspect_ratio_str = f"{aspect_ratio[0]}x{aspect_ratio[1]}"
        output_file_name = f"{base_name}__{aspect_ratio_str}__{i:02d}.mp4"
        output_file_path = os.path.join(output_folder, output_file_name)

        # Write the output video
        snippet.write_videofile(output_file_path, codec="libx264", audio_codec="aac")

    video.close()

if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: ./script_name.py <input_video>")
        sys.exit(1)
